...
cd "$SCRIPT_DIR"
mkdir -p LOGs
QEMU_SOCK=/tmp/orthox-test-cc1.sock
SERIAL_LOG=LOGs/serial.log
rm -f "$QEMU_SOCK" "$SERIAL_LOG"
qemu-system-x86_64 -M pc -m 4G -cdrom out/orthos.iso -boot d -display none \
...
def send_qemu(cmd):
    try:
        ...
        s.connect("/tmp/orthox-test-cc1.sock")
