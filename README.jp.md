...
## ステータス
プロジェクトは現在活発に開発中です。カーネルの主要な機能は安定しており、QEMU user-net 上でのネットワークも動作しています。現在の Orthox-64 は DNS 解決、BusyBox `httpd` による配信、そして BearSSL ベースの userspace クライアントによる実サイトの HTTPS ページ取得まで到達しています。今後はユーザーランド互換性の拡充と、現在の最小 HTTP/HTTPS クライアントの高機能化を進める段階です。
