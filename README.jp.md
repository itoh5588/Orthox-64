...
## デモ動画
- **Boot デモ:** GitHub Releases か外部動画へのリンクに差し替えてください。例: `https://github.com/itoh5588/Orthox-64/releases`
- **DOOM デモ:** キャプチャをアップロードしたあとで、同様に動画リンクへ差し替えてください。
- 収録構成の推奨:
  - boot banner
  - shell prompt
  - `runqstat` や `httpsfetch` などの SMP / network 実証
  - `doom-musl.elf` 起動とゲーム画面
...
あわせて、RISC-V `virt` の bring-up も実用的な到達点まで進みました。Orthox-64 は QEMU `riscv64` 上で起動し、PMM/VMM、first user task、`musl` user ELF 実行まで完了しており、BusyBox の `pwd`、`ls`、`cat`、さらに BusyBox `ash` の起動も確認済みです。現在の RISC-V 側は、この動作する基盤を前提に、shell と userland の安定化を続ける段階です。
