...
/* These depend upon the type of sigset_t, which right now 
   ...
   are not ANSI. */
static __inline__ int __orthos_sigaddset(sigset_t *what, int sig) {
  *what |= ((sigset_t)1 << sig);
  return 0;
}
static __inline__ int __orthos_sigdelset(sigset_t *what, int sig) {
  *what &= ~((sigset_t)1 << sig);
  return 0;
}
static __inline__ int __orthos_sigemptyset(sigset_t *what) {
  *what = (sigset_t)0;
  return 0;
}
static __inline__ int __orthos_sigfillset(sigset_t *what) {
  *what = ~((sigset_t)0);
  return 0;
}
static __inline__ int __orthos_sigismember(const sigset_t *what, int sig) {
  return ((*what) & ((sigset_t)1 << sig)) != 0;
}
#define sigaddset(what,sig)    __orthos_sigaddset((what), (sig))
#define sigdelset(what,sig)    __orthos_sigdelset((what), (sig))
#define sigemptyset(what)      __orthos_sigemptyset((what))
#define sigfillset(what)       __orthos_sigfillset((what))
#define sigismember(what,sig)  __orthos_sigismember((what), (sig))
